...
test('MaidSlimmer：超阈值 JSON tool 输出被骨架化', async () => {
  ...
  const slimmer = new MaidSlimmer(mockCtx(), { 'slim.thresholdChars': 2000 })
...
test('MaidSlimmer：错误类输出保留尾部（诊断信息）', async () => {
  const { MaidSlimmer } = await import('../src/slimmer.mjs')
  const slimmer = new MaidSlimmer(mockCtx(), { 'slim.thresholdChars': 2000, 'slim.headChars': 200, 'slim.tailChars': 200 })
