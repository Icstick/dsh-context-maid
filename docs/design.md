...
> 日期：2026-09-08 | 状态：0.3.0 已收口（C0→M5→M6→M7 全落地，52 测试全绿，版本 0.3.0；设计稿 docs/design-0.3.0.md）——v0.2 基线（P0-1/P0-2 修复集 535d667，36 测试）（用户拍板：命名 dsh-context-maid、repo 在 my-plugins/、工具链 mjs+node --test、sweep aggressive 保留意见、新增 compact 模型用户可配 + 智能路由端点） | 前置调研：
...
## 9.7 实测记录（2026-09-21 live，B12/B14 可观测性）

### 为什么改

`/context-maid status` 原来只显示**最近一次** PIN 校验，形成幸存者偏差——调研者本人就因此误判为
「软保护 100% 有效」（首次只查了最近 12 行，恰好全是高命中）。把「最近一次」换成「近 N 次分布」
之后，本机随即暴露了真实形态。

### 本机实测（19 行 `op=pin`，2026-09-20 12:02 → 2026-09-21 14:58）

```
锚点总数        每次恒为 5（不是文档里的 4）
命中分布        0/5 ×10、1/5 ×4、3/5 ×4、2/5 ×1
全丢（0/N）     10/19 = 53%
可校验率        每次恒报「11 条事实无字面锚点」→ 16 事实抽 5 锚点 ≈ 31%
```

**结论**：软保护的实际强度远低于旧口径给人的印象，而且**每次都有约 7 成事实根本不可校验**
（纯自然语言偏好抽不出字面锚点）——「命中率高」与「保护有效」不是一回事，这正是 B12 要修的口径。

> **口径存疑，未结**：本机 19 行全是 5 锚点，而上游调研记录写的是 38 次、分母恒为 4、全丢仅 10.5%。
> 两者不是同一份样本，已列入待核对项（对齐 `06-measurements/maid-audit-snapshot.csv`），本文不下结论。

### foldDepth

按 `session_id` 反推（旧数据的形态，新代码会直接写在行里）：

```
session-7149728f  10 层      session-f91b0a3e  5 层
session-0d45cbf0   4 层      session-f853c934  2 层
```

深度是「这个会话还要不要继续」的判据，此前只能手写 `group by` 才看得到；现在 `status` 直接给。
阈值 10 起 warn（不阻断）。

### 两处设计偏离（都是被真实风险推着改的）

1. **foldDepth 用 `session.id` 字符串作键，不用 WeakMap 按对象**。原设计稿写 WeakMap（随会话回收、零新依赖），
   但对象身份在事件派发之间没有保证；一旦宿主每次派发都换包装对象，WeakMap 恒返回 0，深度会永远写 1 而且**没有任何报错**。字符串键的代价只是每会话一个整数。
2. **`op=pin` 的 `detail` 落盘前压长度**。审计表把 `detail` 截到 500 字符，而锚点列表是变长的：
