...
- MAID-B16（审计读取行形状）✅ 2026-09-25 —— `audit.recent()` 增归一化，补齐 `sessionId` / `tokensBefore` / `tokensAfter`（只增不删）；B14 的「从审计库播种 foldDepth」从**恒不生效**恢复为生效。真库对照：修复前 11 个会话恒为 1，修复后折了 39 次的会话接续到 40。新增 `test/m8.test.mjs` 走真 SQLite 往返（老测试喂 mock 行故未抓住）
