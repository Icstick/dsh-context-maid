...
| 0.3.0 | — | C0/M5/M6 已收口 → M7 收口（死键删除 + B10 预算 + version bump）剩余；设计稿 docs/design-0.3.0.md（2026-09-08 定稿） |
...
| MAID-B9 | 计划 | 死键全景（扩表）：trigger.minTokens / trigger.eventSlim / sweep.enabled / sweep.aggressive / pin.inject / summarization.allowLocal（eventSlim 为 B9 原漏登） | 2026-09-08 定：eventSlim 随 B8 激活（M5）；sweep.* 随 B7 激活（M6）；minTokens + pin.inject + allowLocal 删键（M7） | decided（eventSlim 已激活 M5、sweep.* 已激活 M6；trigger.minTokens + pin.inject + allowLocal 删除待 M7） |
...
- MAID-B7（sweep 执行器）✅ M6 —— scanSweepCandidates 真实事件模型重构（surface 视角 + callId 配对）+ stubToolResultNode（compaction/prune 协议）+ engine ② sweep 节流清扫 + op=sweep 审计；sweep.enabled 激活（aggressive 规则未扩展，保留开关）；48 测试全绿（0.3.0 设计稿 §4）