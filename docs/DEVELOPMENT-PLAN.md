...
| 0.3.0 | — | C0/M5 已收口 → M6 sweep（B7）→ M7 收口（死键删除 + B10）剩余；设计稿 docs/design-0.3.0.md（2026-09-08 定稿） |
...
| MAID-B9 | 计划 | 死键全景（扩表）：trigger.minTokens / trigger.eventSlim / sweep.enabled / sweep.aggressive / pin.inject / summarization.allowLocal（eventSlim 为 B9 原漏登） | 2026-09-08 定：eventSlim 随 B8 激活（M5）；sweep.* 随 B7 激活（M6）；minTokens + pin.inject + allowLocal 删键（M7） | decided（eventSlim 已激活 M5；sweep.* 待 M6；三死键删除待 M7） |
