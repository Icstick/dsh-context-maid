...
| 0.3.0 | — | B7 sweep 执行器 + B8 SLIM 落地监听（2026-09-08 拍板主线）+ B3/B4 清洁 + B9 键位定夺；B6/B10 视迭代 |
...
| MAID-B7 | 计划 | sweep 执行器：scanSweepCandidates 纯函数全树无调用方（README 已诚实标注"执行器未接线"） | 0.3.0 主线：实现（sweep.enabled 语义落地，aggressive 审批门视设计定） | decided（2026-09-08 拍板：0.3.0 主线实现） |
| MAID-B8 | 计划 | SLIM"落地即瘦身"监听：当前仅官方折叠压力路径内 pruneSession 生效（设计意图 = tool 输出落地即瘦身） | 0.3.0 主线：tool 输出落地即瘦身独立监听 | decided（2026-09-08 拍板：0.3.0 主线实现） |
| MAID-B9 | 计划 | 保留键三件：trigger.minTokens（低于此不触发暂无实现）、pin.inject（逐轮注入未接线）、sweep.enabled/aggressive（同 B7） | sweep.enabled/aggressive 随 B7 实现；trigger.minTokens / pin.inject 待 0.3.0 设计稿定实现 or 删键 | split（2026-09-08） |
